...
void lumiWeight(int mctype){
  ...
  for(it = list_mc_files.begin(); it != list_mc_files.end(); it++){
    ...
    TString Stringing = MCSamplemap[filename];
    tree->Branch("MCSample",&MCSample, 4000,0);
    ...
    TFile *f_save;
    f_save = new TFile("mergedRoot/"+Stringing+".root","RECREATE");
