...
const TString inpath("/data/group/top/CompleteRun/Nov1/fall10systematics-output/");
...
const TString outpath("/data/group/top/CompleteRun/Nov1/fall10systematics-result/");
...
// WARNING: must be false to produce input files for CombinedCrossSection.C!!!
// true only to produce stat/syst/model overview plots
// Bool_t isStat  = kTRUE;
...
void SystematicErrors() {
  ...
  if(isExp) {
	...
	//  TOTAL XS
	if(isTotal) {
	  if( title.Contains("mumu_") )      Sys_Error  = 0.034;
	  if( title.Contains("ee_") )        Sys_Error  = 0.034;
	  if( title.Contains("emu_") )       Sys_Error  = 0.034;
	  if( title.Contains("combined_") )  Sys_Error  = 0.034;
	}

	//  DIFFERENTIAL XS
	if(isDiff) {
	  if( title.Contains("mumu_") )      Sys_Error  = 0.017;
	  if( title.Contains("ee_") )        Sys_Error  = 0.017;
	  if( title.Contains("emu_") )       Sys_Error  = 0.017;
