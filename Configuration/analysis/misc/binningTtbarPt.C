...
TH2F* myHist2d;
  ...
  myHist2d = (TH2F*)myFile1->Get("analyzeTopMatchKinematics/kin_genRecTtbarPt");
...
TH1F* myHist1a = new TH1F( "Projektion" , "Projektion"  ,  100,   0., 200.);
...
for (int k=0; k< binN ; k++)
		...
		cout << "2*Sigma:     "<< 2*sigma[k] << "        Binsize:  "<< (bins[k+1]-bins[k])*2. <<  "      Binsetting:  " <<bins[k]*2. << "-" << bins[k+1]*2. << endl;
 ...
 TCanvas *a=new TCanvas("2DTtbarPt","2DTtbarPt",1);
  // set canvas style 
  gStyle->SetPalette(1);
  myHist2d->Draw("colz");
  myHist2d->GetXaxis()->SetTitle("Ttbar Pt Gen");
  myHist2d->GetYaxis()->SetTitle("Ttbar Pt Rec");
...
// Get Correlation Coefficient
 ...
 // create pdf
  a->SaveAs("kin_genRecTtbarPt.pdf");
