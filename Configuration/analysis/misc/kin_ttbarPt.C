...
// Canvas Style
  ...
  //lese histogramm aus
  myHist1a = (TH1F*)myFile1->Get("analyzeTopGenKinematics/kin_ttbarPt");
  myHist2a = (TH1F*)myFile1->Get("analyzeTopRecKinematics/kin_ttbarPt");
  myHist3a = (TH1F*)myFile1->Get("analyzeTopMatchKinematics/kin_ttbarPt");
...
for (int i=1; i< myHist3a->GetNbinsX()+1 ; i++ )
  ...
  myHist1a->GetXaxis()->SetTitle("kin_ttbarPt");
  myHist1a->GetYaxis()->SetTitle("event density");
  myHist1a->SetTitle("kin_ttbarPt");  
...
legb->Draw("box");
  ...
  a->SaveAs("kin_ttbarPt.pdf");
  b->SaveAs("kin_ttbarPt2.pdf");	
  return 0;
}
