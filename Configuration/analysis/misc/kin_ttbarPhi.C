...
// Canvas Style
  ...
  //lese histogramm aus
  myHist1a = (TH1F*)myFile1->Get("analyzeTopGenKinematics/kin_ttbarPhi");
  myHist2a = (TH1F*)myFile1->Get("analyzeTopRecKinematics/kin_ttbarPhi");
  myHist3a = (TH1F*)myFile1->Get("analyzeTopMatchKinematics/kin_ttbarPhi");
...
for (int i=1; i< myHist3a->GetNbinsX()+1 ; i++ )
  ...
  myHist1a->GetXaxis()->SetTitle("kin_ttbarPhi");
  myHist1a->GetYaxis()->SetTitle("event density");
  myHist1a->SetTitle("kin_ttbarPhi");  
...
legc->Draw("box");
  ...
  a->SaveAs("kin_ttbarPhi.pdf");
b->SaveAs("kin_ttbarPhi2.pdf");
