...
for (int k=0; k< binN ; k++)
 		{
		cout << "2*Sigma:     "<< 2*sigma[k] << "        Binsize:  "<< (bins[k+1]-bins[k])*3 <<  "      Binsetting:  " << bins[k]*3 << "  -  "<< bins[k+1]*3 << endl; 
		}
 ...
 TCanvas *a=new TCanvas("2DtopPt","2DtopPt",1);
  // set canvas style 
  gStyle->SetPalette(1);
  myHist2d->Draw("colz");
  myHist2d->GetXaxis()->SetTitle("Top Pt Gen /GeV");
  myHist2d->GetYaxis()->SetTitle("Top Pt Rec /GeV");
