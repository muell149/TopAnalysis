...
// Canvas Style
  ...
  //lese histogramm aus
  myHist1a = (TH1F*)myFile1->Get("analyzeTopGenKinematics/kin_ttbarMass");
  myHist2a = (TH1F*)myFile1->Get("analyzeTopRecKinematics/kin_ttbarMass");
  myHist3a = (TH1F*)myFile1->Get("analyzeTopMatchKinematics/kin_ttbarMass");
...
for (int i=1; i< myHist3a->GetNbinsX()+1 ; i++ )
  ...
  myHist1a->GetXaxis()->SetTitle("kin_ttbarMass");
  myHist1a->GetYaxis()->SetTitle("event density");
  myHist1a->SetTitle("kin_ttbarMass");  
...
legb->Draw("box");
  ...
  a->SaveAs("kin_ttbarMass.pdf");
  b->SaveAs("kin_ttbarMass2.pdf");
