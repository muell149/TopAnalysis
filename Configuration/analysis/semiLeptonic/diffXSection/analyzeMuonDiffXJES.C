...
enum styles {kSig, kWjets, kBkg, kZjets, kQCD, kSigJES11, kSigJES09, kPseudo50, kSGBG};
...
void analyzeMuonDiffXJES()
{
  ...
  // stays the same
  // for current 7TeV Mc@Nlo sample (shifted signal, full statistics)
  ...
  // ---
  //    diffXSec plot for signal + background for datalike signature
  // ---
  
  // a) creating
  pt_.push_back( (TH1F*)(ptEventNumbers_[kSig]->Clone()) );

  for(unsigned int idx=1; idx<files_.size()-3; ++idx) {
  ...
  std::cout << "( ttbarsig / ttbar bkg / W+jets / Z+jets / QCD )::"<< std::endl;
  textHelper = "(weighted) event composition per pt-bin";
  textHelper += "( ttbarsig / ttbar bkg / W+jets / Z+jets / QCD )::";
  output_ .push_back(textHelper);

  for(int i=2; i<= pt_[kSig]->GetNbinsX()+1; i++){
    std::cout << "pt-bin "<< i << ": ( " <<  ptEventNumbers_[kSig]->GetBinContent(i) << " / " <<  ptEventNumbers_[kBkg]->GetBinContent(i) <<" / " << ptEventNumbers_[kWjets]->GetBinContent(i) <<" / " <<   ptEventNumbers_[kZjets]->GetBinContent(i)  <<" / " <<   ptEventNumbers_[kQCD]->GetBinContent(i) <<" ) " <<std::endl;
  ...
  std::vector<double> ptDiffXError_, ptDiffXPseudoError_;
  ...
  // create a legend relative for stack plot with underground
  TLegend *leg0 = new TLegend(0.48, 0.69, 0.95, 0.94);
  leg0->SetFillStyle(0);
  leg0->SetBorderSize(0);
  leg0->SetHeader("MC samples @ 7TeV");
  leg0->AddEntry( ptEventNumbersRel_[kSig]       , "semi #mu ttbar Mc@Nlo", "L");
  leg0->AddEntry( ptEventNumbersRel_[kBkg]       , "other ttbar Mc@Nlo"   , "L");
  leg0->AddEntry( ptEventNumbersRel_[kQCD]       , "QCD Pythia"           , "L");
  leg0->AddEntry( ptEventNumbersRel_[kWjets]     , "Wjets Madgraph"       , "L");
  leg0->AddEntry( ptEventNumbersRel_[kZjets]     , "Zjets Madgraph"       , "L");

  // create a legend for Signal, Signal + Background, pseudo data
  TLegend *leg5 = new TLegend(0.35, 0.65, 0.95, 0.9);
  leg5->SetFillStyle(0);
  ...
  pt_ [kSig]->SetMaximum(1.5* pt_ [kSigJES11]->GetMaximum() );
  ...
  ptEventNumbers_ [kSigJES11]->SetLineColor (kBlue);
  ptEventNumbers_ [kSigJES11]->SetLineStyle(3);
  ptEventNumbers_ [kSigJES11]->Draw("same");
  ptEventNumbers_ [kSigJES09]->SetLineColor (kBlack);
  ptEventNumbers_ [kSigJES09]->SetLineStyle(2);
  ptEventNumbers_ [kSigJES09]->Draw("same");
  leg3                       ->Draw("same");
  ...
  ptEventNumbersRel_[kSig]->Draw("AXIS");
  ...
  // draw canvas
  canv10->cd(0);
  canv10->SetTitle("detail numbers");
  leg1->Draw("");
  leg2->Draw("same");
...
//   canv10->Print("./diffXSecFromSignal/plots/ptOutputJESandBGMcatnlo7TeV.png");
