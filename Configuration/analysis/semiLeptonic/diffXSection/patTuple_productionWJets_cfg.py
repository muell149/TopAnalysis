...
## special: embeds genParticles muons which are only from W decay
...
## select only muons from W decay from genParticles (genWMuons)
from TopAnalysis.TopUtils.GenCandSelector_cfi import isolatedGenMuons
process.genWMuons = isolatedGenMuons.clone(
...
process.muonMatchW = process.muonMatch.clone(
     #src = 'muons',
     matched = 'genWMuons',
     mcStatus = [3]
)
process.patDefaultSequence.replace(process.muonMatch, process.muonMatchW)
process.patMuons.genParticleMatch = cms.VInputTag(
    cms.InputTag("muonMatchW")
)
