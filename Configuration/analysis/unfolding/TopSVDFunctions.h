...
// the result is zero.
double SVD_Divide(double numerator, double denominator)
{
    double result = 0.;
...
double SVD_BinomialError(double numerator, double denominator)
{
...
void SVD_MoveOFBins1D(TH1D* hist){
...
void SVD_MoveOFBins2D(TH2D* hist){
...
void SVD_EmptySideBins1D(TH1D* histo) 
{
...
// Histogram side bins are also emptied.
void SVD_EmptySideBins2D(TH2D* histo)
{
...
TH1D* SVD_Rebin1D(TH1* input, int nbinsx, double* binsx)
{
...
TH2D* SVD_Rebin2D(TH1* input, int nbinsx, double* binsx, int nbinsy, double* binsy, bool transpose = false)
{
    // Create new histo on the heap
    TString nameNewHist = input->GetName();
    nameNewHist.Append("Rebinned");
...
TH2D* SVD_Cov2Corr(TH2D* input, TString name, TString axisName, TString type)
{
            ...
            if ( i != j ) {
                if ( errsq1 > 0. && errsq2 > 0. ) {
                    ...
                    if ( cov == 0. &&  errsq1 >= 0. && errsq2 >= 0. && (errsq1 == 0. || errsq2 == 0.)) {
                        ...
                        cout << "*********************************************************" << endl;
                        cout << "WARNING! SVD_Cov2Corr()" << endl;
...
TH1D* SVD_Cov2Err(TH2D* input, TH1D* means, TString name, TString axisName, TString type)
{
...
void SVD_Draw2D(TH2D* hist, TString options = "")
{
    // Set the Style	
...
void SVD_Draw1D(TH1D* hist, TString options = "",  int color = 1)
{
    // Use current style
    hist->UseCurrentStyle();
...
void SVD_DrawRange(TH1D* histo, double ymin, double ymax, TString options = "",  int color = 0)
{
    // Clone Histo
...
TH1D* SVD_MakeRatioNum(TH1D* numerator, TH1D* denominator)
{
...
// Boundaries: If ymax < ymin, then range will be set automatically
TH1D* SVD_MakeRatioDenom(TH1D* numerator, TH1D* denominator)
{
...
void SVD_DrawStack(THStack* stack, TString xTitle, TString yTitle, TString options = "", int cols = -1)
{
...
// Set Name, Title and Axis Titles
// note: The Title is set to the value of name
void SVD_SetTitles2D(TH2D* histo, TString name, TString xTitle, TString yTitle, TString zTitle)
{
    histo->SetName(name);
    histo->SetTitle(name);
    histo->GetXaxis()->SetTitle(xTitle);
...
double SVD_Chi2A(TH1D* dataHist, TH1D* refoldHist)
{
    double chi2a = 0.;
...
int SVD_GoodRecBins(TH1D* dataHist)
{
    int numbins = 0;
    for ( int i = 1 ; i <= dataHist->GetNbinsX(); i++) {
        double dataValue = dataHist->GetBinContent(i);
...
int SVD_GoodGenBins(TH1D* xiniHist)
{
    int numbins = 0;
...
// Calculate NDOF
int SVD_Ndof(TH1D* dataHist, TH1D* xiniHist)
{
