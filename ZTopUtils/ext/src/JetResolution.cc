...
//using namespace std; //had to be moved!!!
...
//______________________________________________________________________________
JetResolution::JetResolution(const std::string& fileName, bool doGaussian) :
        resolutionFnc_(0) {
    using namespace std;
...
//______________________________________________________________________________
void JetResolution::initialize(const std::string& fileName, bool doGaussian) {
    using namespace std;
    size_t pos;
    ...
    if (doGaussian)
        ...
        resolutionFnc_ = new TF1(fncname.c_str(), fnc_gaussalpha1alpha2, -5.,
                5., 5);
    ...
    while (!tmp.empty()) {
        ...
        if (!doGaussian || paramAsStr == "mean" || paramAsStr == "sigma") {
            parameters_.push_back(
                    new JetCorrectorParameters(fileName, paramAsStr));
            formula = parameters_.back()->definitions().formula();
            parameterFncs_.push_back(
                    new TF1(("f" + paramAsStr + "_" + name()).c_str(),
                            formula.c_str(),
                            parameters_.back()->record(0).parameters()[0],
                            parameters_.back()->record(0).parameters()[1]));
            resolutionFnc_->SetParName(nPar,
...
TF1* JetResolution::resolution(const std::vector<float>& x,
    ...
    for (unsigned iPar = 0; iPar < parameters_.size(); iPar++) {
        ...
        for (unsigned i = 0; i < N; i++)
            yy[i] = (y[i] < pars[2 * i]) ? pars[2 * i] :
                    (y[i] > pars[2 * i + 1]) ? pars[2 * i + 1] : y[i];
...
//______________________________________________________________________________
TF1* JetResolution::parameterEta(const std::string& parameterName, float eta) {
    using namespace std;
    vector<float> x;
    x.push_back(eta);
...
TF1* JetResolution::parameter(const std::string& parameterName,
    ...
    for (unsigned i = 0; i < parameterFncs_.size() && result == 0; i++) {
        ...
        if (fncname.find("f" + parameterName) == 0) {
            ...
            for (unsigned ii = 0; ii < x.size(); ii++)
                ssname << "_" << parameters_[i]->definitions().binVar(ii)
                        << x[ii];
            ...
            const std::vector<float>& pars =
                    parameters_[i]->record(bin).parameters();
    ...
    if (0 == result)
        cerr << "JetResolution::parameter() ERROR: no parameter "
...
double fnc_dscb(double*xx, double*pp) {
    ...
    double u = (x - mu) / sig;
    double A1 = TMath::Power(p1 / TMath::Abs(a1), p1)
            * TMath::Exp(-a1 * a1 / 2);
    double A2 = TMath::Power(p2 / TMath::Abs(a2), p2)
            * TMath::Exp(-a2 * a2 / 2);
...
double fnc_gaussalpha(double *v, double *par) {
    ...
    return (t <= cut) ?
            N * TMath::Exp(-0.5 * t * t) :
            N * TMath::Exp(-0.5 * (alpha * (t - cut) + cut * cut));
}
...
double fnc_gaussalpha1alpha2(double *v, double *par) {
    ...
    return (t <= cut) ? N * TMath::Exp(-0.5 * t * t) :
           ((v[0] - mean) >= 0) ?
                   N * TMath::Exp(-0.5 * (alpha1 * (t - cut) + cut * cut)) :
