...
// $Id: JetCorrectorParameters.cc,v 1.1 2013/04/04 13:47:26 jkiesele Exp $
//
...
JetCorrectorParameters::Definitions::Definitions(const std::string& fLine) {
    ...
    if (!tokens.empty()) {
        if (tokens.size() < 6) {
            ...
            sserr << "(line " << fLine << "): less than 6 expected tokens:"
                    << tokens.size();
...
int JetCorrectorParameters::neighbourBin(unsigned fIndex, unsigned fVar,
    ...
    for (unsigned i = 0; i < size(); ++i) {
        ...
        if (tmp == N - 1) {
            if (fNext)
                if (fabs(record(i).xMin(fVar) - record(fIndex).xMax(fVar))
                        < 0.0001)
                    tmp += 1;
            if (!fNext)
                if (fabs(record(i).xMax(fVar) - record(fIndex).xMin(fVar))
                        < 0.0001)
                    tmp += 1;
        }
        if (tmp == N) {
...
void JetCorrectorParameters::printScreen() const {
    ...
    if (definitions().isResponse())
        std::cout << "Type (Response or Correction): " << "Response"
                << std::endl;
    else
        std::cout << "Type (Response or Correction): " << "Correction"
                << std::endl;
...
const char * JetCorrectorParametersCollection::labelsArray_[JetCorrectorParametersCollection::N_LEVELS] =
        { "L1Offset", "L2Relative", "L3Absolute", "L4EMF", "L5Flavor", "L6UE",
                "L7Parton", "L1JPTOffset", "L2L3Residual", "Uncertainty",
                "L1FastJet", "UncertaintyAbsolute", "UncertaintyHighPtExtra",
                "UncertaintySinglePion", "UncertaintyFlavor", "UncertaintyTime",
                "UncertaintyRelativeJEREC1", "UncertaintyRelativeJEREC2",
                "UncertaintyRelativeJERHF", "UncertaintyRelativeStatEC2",
                "UncertaintyRelativeStatHF", "UncertaintyRelativeFSR",
                "UncertaintyPileUpDataMC", "UncertaintyPileUpOOT",
                "UncertaintyPileUpPt", "UncertaintyPileUpBias",
                "UncertaintyPileUpJetRate" };

const char * JetCorrectorParametersCollection::l5FlavorArray_[JetCorrectorParametersCollection::N_L5_SPECIES] =
        { "L5Flavor_bJ", "L5Flavor_cJ", "L5Flavor_qJ", "L5Flavor_gJ",
                "L5Flavor_bT", "L5Flavor_cT", "L5Flavor_qT", "L5Flavor_gT" };

const char * JetCorrectorParametersCollection::l7PartonArray_[JetCorrectorParametersCollection::N_L7_SPECIES] =
        { "L7Parton_gJ", "L7Parton_qJ", "L7Parton_cJ", "L7Parton_bJ",
                "L7Parton_jJ", "L7Parton_qT", "L7Parton_cT", "L7Parton_bT",
                "L7Parton_tT" };

std::vector<std::string> JetCorrectorParametersCollection::labels_(labelsArray_,
...
void JetCorrectorParametersCollection::push_back(key_type i,
        value_type const & j, label_type const & flav) {
...
JetCorrectorParameters const & JetCorrectorParametersCollection::operator[](
        key_type k) const {
    ...
    std::cerr << ("InvalidInput") << " cannot find key " << static_cast<int>(k)
            ...
            << std::endl;
    return iend->second; // just to have some return
}
...
void JetCorrectorParametersCollection::validKeys(
    ...
    keys.clear();
    for (collection_type::const_iterator ibegin = corrections_.begin(), iend =
            corrections_.end(), i = ibegin; i != iend; ++i) {
        keys.push_back(i->first);
    }
    for (collection_type::const_iterator ibegin = correctionsL5_.begin(), iend =
            correctionsL5_.end(), i = ibegin; i != iend; ++i) {
        keys.push_back(i->first);
    }
    for (collection_type::const_iterator ibegin = correctionsL7_.begin(), iend =
            correctionsL7_.end(), i = ibegin; i != iend; ++i) {
        keys.push_back(i->first);
    }
}

// Find the L5 bin for hashing
JetCorrectorParametersCollection::key_type JetCorrectorParametersCollection::getL5Bin(
        std::string const & flav) {
    std::vector<std::string>::const_iterator found = find(l5Flavors_.begin(),
            l5Flavors_.end(), flav);
...
JetCorrectorParametersCollection::key_type JetCorrectorParametersCollection::getL7Bin(
        std::string const & flav) {
    std::vector<std::string>::const_iterator found = find(l7Partons_.begin(),
            l7Partons_.end(), flav);
...
JetCorrectorParametersCollection::key_type JetCorrectorParametersCollection::findKey(
        std::string const & label) const {

    // First check L5 corrections
    std::vector<std::string>::const_iterator found1 = find(l5Flavors_.begin(),
            l5Flavors_.end(), label);
    if (found1 != l5Flavors_.end()) {
        return getL5Bin(label);
    }

    // Next check L7 corrections
    std::vector<std::string>::const_iterator found2 = find(l7Partons_.begin(),
            l7Partons_.end(), label);
    if (found2 != l7Partons_.end()) {
        return getL7Bin(label);
    }

    // Finally check the default corrections
    std::vector<std::string>::const_iterator found3 = find(labels_.begin(),
            labels_.end(), label);
    ...
    std::cerr << ("InvalidInput") << " Cannot find label " << label
            << std::endl;
    return getL7Bin(label); // to avoid warnings
}
