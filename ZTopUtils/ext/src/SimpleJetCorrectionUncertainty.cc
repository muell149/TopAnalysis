...
float SimpleJetCorrectionUncertainty::uncertainty(std::vector<float> fX,
        float fY, bool fDirection) const {
    float result = 1.;
    int bin = mParameters->binIndex(fX);
    if (bin < 0)
        std::cout << ("SimpleJetCorrectionUncertainty")
                << " bin variables out of range" << std::endl;
...
float SimpleJetCorrectionUncertainty::uncertaintyBin(unsigned fBin, float fY,
    ...
    if (fBin >= mParameters->size())
        std::cout << ("SimpleJetCorrectionUncertainty") << " wrong bin: "
                << fBin << ": only " << mParameters->size() << " are available"
                << std::endl;
...
float SimpleJetCorrectionUncertainty::linearInterpolation(float fZ,
        const float fX[2], const float fY[2]) const {
