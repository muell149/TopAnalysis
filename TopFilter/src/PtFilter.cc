...
/// default constructor
PtFilter::PtFilter(const edm::ParameterSet& cfg): ViewFilter(cfg)
{
}

/// filter (worker class):
bool PtFilter::filter(const std::vector<edm::View<reco::Candidate> >& objs)
{
  ...
  for(unsigned int jdx=0; jdx<objs.size(); ++jdx){
	...
	obj!=objs[jdx].end(); ++obj) {
      if( idx<min_.size() ) // check for min Pt as long as vector is long enough
	if( !(obj->pt()>min_[idx]) ) passedOnce=false;
      if( idx<max_.size() ) // check for max Pt as long as vector is long enough
	if( !(obj->pt()<max_[idx]) ) passedOnce=false;
