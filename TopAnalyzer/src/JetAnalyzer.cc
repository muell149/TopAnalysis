...
JetAnalyzer::analyze(const edm::Event& evt, const edm::EventSetup& setup)
{
  ...
  for(edm::View<pat::Jet>::const_iterator jet = jets->begin();jet != jets->end(); ++jet) {
    ...
    if(!jet->isCaloJet()){
      ...
      neutralHadronFrac_  ->Fill(jet->neutralHadronEnergyFraction(), weight);
      chargedEmFrac_      ->Fill(jet->chargedEmEnergyFraction(), weight);
      neutralEmFrac_      ->Fill(jet->neutralEmEnergyFraction(), weight);
      chargedMulti_       ->Fill(jet->chargedMultiplicity(), weight);
      nConst_             ->Fill(jet->nConstituents(), weight);
    }

    bDiscTCHE_ ->Fill(TCHEdiscr , weight);
    bDiscTCHP_ ->Fill(TCHPdiscr , weight);
  ...
  multiTCHPL_ ->Fill(ntagsTCHPL, weight);
  multiTCHPM_ ->Fill(ntagsTCHPM, weight);
  multiTCHPT_ ->Fill(ntagsTCHPT, weight);
  multiSSVHEM_->Fill(ntagsSSVHEM, weight);
  multiSSVHET_->Fill(ntagsSSVHET, weight);
  multiSSVHPM_->Fill(ntagsSSVHPM, weight);
}
