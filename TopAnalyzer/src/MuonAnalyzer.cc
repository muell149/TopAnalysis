...
{ 
  ...
  for(PatMuonCollection::const_iterator muon = muons->begin(); muon!= muons->end(); ++muon) { 
    ...
    dep_ecal_->Fill(muon->isolationR03().emVetoEt );         
    dep_hcal_->Fill(muon->isolationR03().hadVetoEt);     

    iso_comb_ ->Fill((muon->trackIso()+muon->caloIso())/muon->pt()); 
    iso_comb2_->Fill((muon->trackIso()+muon->caloIso())/muon->pt());     
    
    dB_->Fill(muon->dB());
    
    n_mustations_->Fill(muon->numberOfMatches());
    ...
    d0_   ->Fill(muon->track()->d0());
	
    if(muon->isGlobalMuon()){
      chi_norm_ ->Fill(muon->globalTrack()->normalizedChi2());
      n_validmuhits_->Fill(muon->globalTrack()->hitPattern().numberOfValidMuonHits());    
      type_->SetBinContent(2,type_->GetBinContent(2)+1);
    }
    if(muon->isTrackerMuon()){  
      nhits_->Fill(muon->track()->numberOfValidHits());     
      n_pixhits_    ->Fill(muon->track()->hitPattern().numberOfValidPixelHits());
      n_trackerhits_->Fill(muon->track()->hitPattern().numberOfValidTrackerHits());       
