...
//			double jetIsoCorr = mon->getCorrelationFactor(varname + "_vs_JetIso");
			...
			double abs_caloCorrErr = mon->getCorrelationError(varname + "_vs_AbsCalo");
			double abs_trackCorrErr = mon->getCorrelationError(varname + "_vs_AbsTrack");
...
//			summary_jet_iso_->GetXaxis()->SetBinLabel(x, varname.c_str());
			...
			summary_abs_Comb_->SetBinError(x, abs_CombCorrErr);
	...
	void addHistogram(std::string name, unsigned int numberOfBins, double min, double max) {
		...
		histos_.insert(make_pair<string, TH1F*> (name, hist));
...
private:
	...
	TH1F * summary_abs_Calo_, *summary_abs_Track_, *summary_jet_iso_, *summary_abs_Comb_;
	//		TH1F *summary_jet_iso_, *summary_abs_Comb_, *summary_rel_Comb_;
	TH1F *summary_rel_Comb_, *summary_rel_Calo_, *summary_rel_Track_;
	...
	double abs_caloLimits_[2], abs_trackLimits_[2], abs_combLimits_[2], jetIsoLimits_[2];
	double rel_caloLimits_[2], rel_trackLimits_[2], rel_combLimits_[2];
	...
	//	double rel_combIso_, rel_caloIso_, rel_trackIso_, weight_, jetIso_;
};
#endif
