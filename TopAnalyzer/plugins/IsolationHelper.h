...
public:
	...
	void makeSummaryPlots(std::string prefix = "") {
		...
		summary_abs_Calo_ = fs_->make<TH1F> (nam.name(off, abs_caloName.c_str()), nam.name(abs_caloName.c_str()),
				histos_.size(), -1., 1.);
		summary_abs_Track_ = fs_->make<TH1F> (nam.name(off, abs_trackName.c_str()), nam.name(abs_trackName.c_str()),
				histos_.size(), -1., 1.);

		summary_rel_Calo_ = fs_->make<TH1F> (nam.name(off, rel_caloName.c_str()), nam.name(rel_caloName.c_str()),
				histos_.size(), -1., 1.);
		summary_rel_Track_= fs_->make<TH1F> (nam.name(off, rel_trackName.c_str()), nam.name(rel_trackName.c_str()),
				histos_.size(), -1., 1.);
		summary_jet_iso_= fs_->make<TH1F> (nam.name(off, jetIsoName.c_str()), nam.name(jetIsoName.c_str()),
				histos_.size(), -1., 1.);
		summary_abs_Comb_= fs_->make<TH1F> (nam.name(off, rel_combName.c_str()), nam.name(rel_combName.c_str()),
				histos_.size(), -1., 1.);
		summary_rel_Comb_= fs_->make<TH1F> (nam.name(off, abs_combName.c_str()), nam.name(abs_combName.c_str()),
				histos_.size(), -1., 1.);

		for (iter = histos_.begin(); iter != histos_.end(); ++iter) {
			std::string varname = iter->first;
			double abs_caloCorr = mon->getCorrelationFactor(varname + "_vs_AbsCcalo");
	...
	void addHistogram(std::string name, unsigned int numberOfBins, double min, double max) {
		...
		mon->addHist(name + "_vs_AbsCcalo", numberOfBins, min, max, abs_caloBins_, abs_caloLimits_[0],
				abs_caloLimits_[1]);
