...
void FullLepGenAnalyzer::analyze(const edm::Event& evt, const edm::EventSetup& es) {
  ...
  fillGenHistos(TopGen_,    *genTop    );
  fillGenHistos(WplusGen_,  *genWplus  );
  fillGenHistos(BGen_,      *genB      );
  fillGenHistos(LepBarGen_, *genLepBar );
  fillGenHistos(NuGen_,     *genNu     );

  fillGenHistos(TopBarGen_, *genTopBar );
  fillGenHistos(WminusGen_, *genWminus );
  fillGenHistos(BBarGen_,   *genBBar   );
  fillGenHistos(LepGen_,    *genLep    );
  fillGenHistos(NuBarGen_,  *genNuBar  );
  
  fillGenHistos(TtBarGen_,  *genTtBar  );
  fillGenHistos(LepPairGen_,*genLepPair);
  fillGenHistos(JetPairGen_,*genJetPair);    
}
...
FullLepGenAnalyzer::bookGenHistos(edm::Service<TFileService>& fs)
{
  ...
  TtBarGen_.push_back( fs->make<TH1D>(ns.name("TtBarRapidity"), "rapidity (t#bar{t})"   , 62, -3.1,   3.1) );  
...
/// fill histograms
void
FullLepGenAnalyzer::fillGenHistos(std::vector<TH1D*>& histos, const reco::Candidate& candidate)
{
