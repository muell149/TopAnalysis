...
$                analyzeSelMuon                                       $ in within the root file; for edm::Analyzers 
  ...
  rootOutput   = fittedHists.root                                     $ root output file to write histograms to
                                                                      $
  outputDir    = analyzeAllMuon                                       $ output directory in root output file 
$                analyzeSelMuon                                       $
								      $
  outputLabels = allMuon                                              $ output labels for multiple ps/eps files 
$                selMuon                                              $ (separated by blanks)
                                                                      $
                                                                      $
...
$=====================================================================
  ...
  xAxes      = "(E_{t,muon}^{rec}-E_{t,muon}^{true})/E_{t,muon}^{true}";$ has to be givin in " and has to end with;
               "(E_{t,muon}^{rec}-E_{t,muon}^{true})/E_{t,muon}^{true}";
               "(eta_{muon}^{rec}-eta_{muon}^{true})/eta_{muon}^{true}";
               "(eta_{muon}^{rec}-eta_{muon}^{true})/eta_{muon}^{true}";
	       "(phi_{muon}^{rec}-phi_{muon}^{true})/phi_{muon}^{true}";
	       "(phi_{muon}^{rec}-phi_{muon}^{true})/phi_{muon}^{true}";

  yAxes      = "mean of StabilizedGuass";			      $ has to be adapted to evaluation type
	       "sigma of StabilizedGuass";
	       "mean of StabilizedGuass";
	       "sigma of StabilizedGuass";
	       "mean of StabilizedGuass";
	       "sigma of StabilizedGuass";

  legEntries = "all muons";                                           $ should contain a legend entry for each 
...
$=====================================================================
  ...
  fitUpperBound    = 2.                                               $ upper boundary of fit range; to be adjusted
                                                                      $ for ratio and difference
  xAxesFit         = "(E_{t,muon}^{rec}-E_{t,muon}^{true})/E_{t,muon}^{true}";$ axes labels for fit histograms; keep dif-
                     "(eta_{muon}^{rec}-eta_{muon}^{true})/eta_{muon}^{true}";$ ferent labels here to adress them in the
                     "(phi_{muon}^{rec}-phi_{muon}^{true})/phi_{muon}^{true}";$ following variable (titleIndex), should be
									      $ in " and end with ;
                     							      $
  yAxesFit         =                                                  $ following variable (titleIndex), should be
                                                                      $ in " and end with ;
  titleIndex       = 0 0 0 0 0 0                                      $ titleIndex of x/yAxesFit for each if the
                     1 1 1 1 1 1                                      $ the fit histograms
                     2 2 2 2                                          $
